...
<x-inputs.wrapper :label="$question->label" :name="$inputName" :required="$question->is_required" :helper-text="$question->helper_text">
    ...
    @if ($value)
        <div class="flex items-center justify-between p-3 mt-2 bg-blue-50 border border-blue-200 rounded-md">
            <div class="flex items-center space-x-2 overflow-hidden">
                ...
                <div class="flex flex-row">
                    ...
                    <a href="{{ Storage::url($value) }}" target="_blank"
                        class="text-sm text-blue-600 underline truncate hover:text-blue-800">
                        Ver archivo
                    </a>
                </div>
            </div>
        ...
        <p class="text-xs text-gray-500 mb-1">Si deseas reemplazar el archivo, selecciona uno nuevo arriba:</p>
    @endif
    @if ($rawFormats)
